...
// 来源地址：http://bxl766.blog.163.com/blog/static/61099505201003112230463/
// --------------------------------------------------------
...
class ClassHelperDemo
{
    ...
    {
        //  演示一：动态生成类。#region 演示一：动态生成类。
        //生成一个类t。
        Type t = DynamicCreateClass.BuildType("MyClass");


        //    演示二：动态添加属性到类。#region 演示二：动态添加属性到类。
        //先定义两个属性。
        List<DynamicCreateClass.CustPropertyInfo> lcpi = new List<DynamicCreateClass.CustPropertyInfo>();
        DynamicCreateClass.CustPropertyInfo cpi;

        cpi = new DynamicCreateClass.CustPropertyInfo("System.String", "S1");
        lcpi.Add(cpi);
        cpi = new DynamicCreateClass.CustPropertyInfo("System.String", "S2");
        lcpi.Add(cpi);



        //再加入上面定义的两个属性到我们生成的类t。
        t = DynamicCreateClass.AddProperty(t, lcpi);
        ...
        //再定义两个属性。
        lcpi.Clear();
        cpi = new DynamicCreateClass.CustPropertyInfo("System.Int32", "I1");
        lcpi.Add(cpi);
        cpi = new DynamicCreateClass.CustPropertyInfo("System.Int32", "I2");
        lcpi.Add(cpi);
        ...
        // 演示三：动态从类里删除属性。#region 演示三：动态从类里删除属性。
        //把'S1'属性从类t中删除。
        t = DynamicCreateClass.DeleteProperty(t, "S1");
        //显示出来，可以看到'S1'不见了。
        DispProperty(t);


        //演示四：动态获取和设置属性值。#region 演示四：动态获取和设置属性值。
        //先生成一个类t的实例o。
        object o = DynamicCreateClass.CreateInstance(t);

        //给S2,I2属性赋值。
        DynamicCreateClass.SetPropertyValue(o, "S2", "abcd");
        DynamicCreateClass.SetPropertyValue(o, "I2", 1234);

        //再把S2,I2的属性值显示出来。
        Console.WriteLine("S2 = {0}", DynamicCreateClass.GetPropertyValue(o, "S2"));
        Console.WriteLine("I2 = {0}", DynamicCreateClass.GetPropertyValue(o, "I2"));
